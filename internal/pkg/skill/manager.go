...
func (m *Manager) BuildLevel1PromptForAgent(agentName string) string {
	...
	for _, s := range skills {
		...
		sb.WriteString(fmt.Sprintf("- **%s**: %s\n  - 物理根目录: `%s`\n  - SOP文件路径: `%s`\n", s.Frontmatter.Name, s.Frontmatter.Description, s.Path, skillFilePath))
	}

	sb.WriteString("\n### 🎯 技能触发、路径判断与自愈重试规范 (CRITICAL):\n")
	sb.WriteString("1. **自动意图识别**：仔细分析用户提问。一旦用户的诉求符合某种技能的描述（如网页提取、代码扫描等），必须优先选择并触发该技能。\n")
	sb.WriteString("2. **装载 SOP 步骤**：在执行具体任务前，你**必须（MUST）**先调用 `read_files` 工具，读取该技能的 `SOP文件路径` (`SKILL.md`)。\n")
