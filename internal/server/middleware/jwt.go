...
// JwtWithSigningMethod with signing method option.
func JwtWithSigningMethod(method jwt.SigningMethod) Option {
	return func(o *options) {
		o.signingMethod = method
	}
}

// JwtWithClaims with customer claim
// If you use it in JwtCheck, f needs to return a new jwt.Claims object each time to avoid concurrent write problems
// If you use it in Client, f only needs to return a single object to provide performance
func JwtWithClaims(f func() jwt.Claims) Option {
	return func(o *options) {
		o.claims = f
	}
}
...
// JwtCheck is a server auth middleware. Check the token and extract the info from token.
func JwtCheck(ctx context.Context, keyFunc jwt.Keyfunc, opts ...Option) (context.Context, error) {
	o := &options{
	...
	if header, ok := transport.FromServerContext(ctx); ok {
		...
		if err != nil {
			...
			if !ok {
				return ctx, errors.Unauthorized(reason, err.Error())
			}
			...
			return ctx, ErrTokenParseFail
		}
		if !tokenInfo.Valid {
			return ctx, ErrTokenInvalid
		}
		if tokenInfo.Method != o.signingMethod {
			return ctx, ErrUnSupportSigningMethod
		}
		ctx = NewContext(ctx, tokenInfo.Claims)
		return ctx, nil
	}
	return ctx, ErrWrongContext
}
...
// JwtFromContext extract auth info from context
func JwtFromContext(ctx context.Context) (token jwt.Claims, ok bool) {
