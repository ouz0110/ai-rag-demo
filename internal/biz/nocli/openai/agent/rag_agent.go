...
func (a *RAGAgent) SystemPrompt(workDir string) string {
	corePrompt := fmt.Sprintf(`你是一个专业的 RAG 知识库问答 Agent 助手，当前工作目录为：%s。
...
4. 【文档探索】若需要检索或探索本地知识库文件，优先使用 rag_search 检索向量库，禁止直接读取文件或执行终端命令。`, workDir)
