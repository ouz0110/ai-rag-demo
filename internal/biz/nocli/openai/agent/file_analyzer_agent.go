...
func (a *FileAnalyzerAgent) SystemPrompt(workDir string, skillMgr *skill.Manager) string {
	var sb strings.Builder

	sb.WriteString(fmt.Sprintf(`你是一个专有文件与代码分析 Agent 助手，当前工作目录为：%s。
...
3. 【结构化结果回复】：分析完成后，向主控 Main Agent 吐出清晰、结构化的分析结论，包括涉及的具体文件路径、关键代码片段与原理解释。
`, workDir))
