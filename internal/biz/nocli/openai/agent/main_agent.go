...
2. 【专有 Agent 委派 (CRITICAL)】：
   ...
   - 【知识库问答任务 (RAG 禁忌原则)】：RAG 检索绝不是默认自动触发的！**严禁在无必要时盲目触发 RAG 检索**。只有当用户在问题中明确指示“检索知识库”、“查阅业务文档”或显式启用 RAG 功能时，才允许调用 'delegate_to_rag_agent' 委派给 RAG 知识库 Agent。普通聊天与常规代码问题直接回答即可。
3. 【基础工具使用】：
