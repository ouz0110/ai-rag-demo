...
export const useKBStore = defineStore('kb', () => {
  ...
  const defaultKB = computed(() => {
    return kbs.value.find((k) => k.is_default) || null;
  });

  // 获取自定义知识库列表
  const customKBs = computed(() => {
    return kbs.value.filter((k) => !k.is_default);
  });

  // 当前选中知识库的详细 Model
  const activeKB = computed(() => {
    return kbs.value.find((k) => k.kb_id === activeKbId.value) || null;
  });
  ...
  async function fetchKBs() {
    ...
    try {
      ...
      if (res && res.kbs) {
        ...
        const exists = res.kbs.some((k) => k.kb_id === activeKbId.value);
        if (!exists && res.kbs.length > 0) {
          const def = res.kbs.find((k) => k.is_default);
          const target = def || res.kbs[0];
  ...
  async function createKB(name: string, description: string) {
    ...
    try {
      ...
      if (res && res.kb) {
        kbs.value.unshift(res.kb);
        activeKbId.value = res.kb.kb_id;
        activeKbTenantId.value = res.kb.tenant_id || 'default_tenant';
        await fetchDocuments(res.kb.kb_id);
        return res.kb;
      }
    } catch (err: any) {
  ...
  async function removeKB(kb_id: string) {
    ...
    try {
      ...
      kbs.value = kbs.value.filter((k) => k.kb_id !== kb_id);
