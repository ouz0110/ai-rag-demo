<template>
  <div class="cot-box border border-indigo-500/20 bg-indigo-950/20 rounded-xl mb-3 overflow-hidden">
    <!-- 头部栏 (点击切换折叠) -->
    <div
      class="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-indigo-900/30 transition-colors select-none"
    ...
    >
      <div class="flex items-center gap-2 text-xs font-medium text-indigo-300">
        ...
        <span>{{ isStreaming ? 'DeepSeek 正在思考...' : '已深度思考' }}</span>
    ...
    <div
      v-show="isExpanded"
      class="px-3.5 py-2.5 text-xs text-indigo-200/80 leading-relaxed border-t border-indigo-500/10 font-mono whitespace-pre-wrap bg-black/20"
    >
      {{ reasoningText }}
